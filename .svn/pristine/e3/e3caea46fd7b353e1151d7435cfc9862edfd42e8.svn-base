...
<body class="gray-bg">
						...
						<div id="toolbar" class="btn-group">
							<div class="pull-left form-inline form-group">
								<select class="form-control" name="HD_ID" id="HD_ID" value="${pd.HD_ID}"
                     			...
                     			<select class="form-control" name="HE_AREA" id="HE_AREA" value="${pd.HE_AREA}" onchange="changePcode();"
                                    ajax--url="dic/getDicAreaForSelect" ajax--column="ID,TEXT" style="width:160px;">
                                    <option value="">--选择地域信息--</option>
                     			</select>
                     			<select class="form-control required" name="caa_pcode" id="caa_pcode" value="${pd.caa_pcode}"
                                    	 ajax--url="dic/getSubDicAreaListByProvinceCode" ajax--column="ID,TEXT" style="width:160px;">
	...
	<script type="text/javascript">
		...
		//表格请求及数据
		var tableColumns = {
			url: 'hospitalExpertTmp/pageSearchs?tm=' + new Date().getTime(),
			toolbar: '#toolbar',
			columns: [{
		        field: 'HEL_ID',
		        visible: false,
		...
		function toEdit(){
			...
			{
 				this.location.href="<%=basePath%>hospitalExpertTmp/toEdit?HEL_ID="+ids;
 				layer.full(
						layer.open({
						    type: 2,
						    title: '修改专家',//窗体标题
						    ...
						    scrollbar: false,//整个页面滚动条是否显示
			 			    content: ["hospitalExpertTmp/toEdit?HEL_ID="+ids , "yes"],//URL地址,yes或者or是表示是否允许出现滚动条 
						    closeBtn: 1 //不显示关闭按钮
						})
					);
			} 
