					...
					<div class="ibox-content">
						<form id="ArticleHealthForm" name="ArticleHealthForm" class="form-horizontal" method="post">
							<div class="form-group">
							    ...
							    <div class="col-sm-4">
									<input type="text" class="form-control required" name="EXPERT_NAME" id="EXPERT_NAME" value="${data.rows[0].onum}" readOnly/>
								</div>
								<label class="col-sm-2 control-label">下单时间：</label>
							    <div class="col-sm-4">
									<input type="text" class="form-control required" name="EXPERT_DUTY" id="EXPERT_DUTY" value="${data.rows[0].otime}" readOnly/>
								</div>
							</div>
							<div class="form-group">
								<label class="col-sm-2 control-label">订单类型：</label>
							    <div class="col-sm-4">
									<input type="text" class="form-control required" name="EXPERT_HOSP" id="EXPERT_HOSP" value="${data.rows[0].otype}" readOnly/>
								</div>
								<label class="col-sm-2 control-label">订单状态：</label>
							    <div class="col-sm-4">
									<input type="text" class="form-control required" name="EXPERT_DEPT" id="EXPERT_DEPT" value="${data.rows[0].ostatus}" readOnly/>
								</div>
							</div>
							<div class="form-group">
								<label class="col-sm-2 control-label">注意事项：</label>
							    <div class="col-sm-10">
									<textarea class="form-control" name="PUBLISH_NAME" id="PUBLISH_NAME" rows="5" readonly="readonly">${data.rows[0].ocontent}</textarea> 
								</div>
							</div>
							<div class="form-group">
		...
		function uinameDesc(O_ID){
			layer.open({
			    ...
			    scrollbar: false,//整个页面滚动条是否显示
 			    content: ["agency/orderManagement/uinameDesc?O_ID=" + O_ID, "yes"],//URL地址,yes或者or是表示是否允许出现滚动条 
			    closeBtn: 1 //不显示关闭按钮
			});
		}
		function onumDesc(O_ID){
			layer.open({
			    type: 2,
			    title: '订单详情',//窗体标题
			    area: ['100%', '100%'],//整个窗体宽、高，单位：像素px
			    ...
			    maxmin: true,//最大、小化是否显示
			    scrollbar: false,//整个页面滚动条是否显示
 			    content: ["agency/orderManagement/toLists_onumDesc?O_ID=" + O_ID, "yes"],//URL地址,yes或者or是表示是否允许出现滚动条 
			    closeBtn: 1 //不显示关闭按钮
			});
		}
