...
<body class="gray-bg">
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						...
						<font color="red" size="2">注：如果专家.陪诊拒单，则拒单的专家.陪诊名称为红色，点击可查看拒单理由</font>
	...
	<script type="text/javascript">
			...
			columns: [{
		        ...
		        formatter:function(value,row,index){
		        	...
		        	if(row.HEL_NAME != null && row.HEL_NAME != ''){
		        		if(row.OS_STATUS == '2'){
		        			return "<button type=\"button\" class=\"btn btn-link\" onclick=\"expertLibsDesc('" + row.expertlibsOID + "','"+row.oid+"');\"><font color = \"red\">" + row.HEL_NAME + "</font></button>";
		        		}else{
		        			return "<button type=\"button\" class=\"btn btn-link\" onclick=\"expertLibsDesc('" + row.expertlibsOID + "','"+row.oid+"');\">" + row.HEL_NAME + "</button>";
		        		}
		        	}else{
		        		return "-";	
		        	}
		        ...
		        formatter:function(value,row,index){
		        	if(row.AI_NAME != null && row.AI_NAME != ''){
		        		if(row.OSHA_STATUS == '2'){
		        			return "<button type=\"button\" class=\"btn btn-link\" onclick=\"accompanyDesc('" + row.accompanyinfoOID + "','"+row.oid+"');\"><font color = \"red\">" + row.AI_NAME + "</font></button>";
		        		}else{
		        			return "<button type=\"button\" class=\"btn btn-link\" onclick=\"accompanyDesc('" + row.accompanyinfoOID + "','"+row.oid+"');\">" + row.AI_NAME + "</button>";
		        		}
		        	}else{
		        		return "-"
		        	}
		...
		//浏览专家
		function expertLibsDesc(expertlibsOID,O_ID){
			if(expertlibsOID != null && expertlibsOID != ""){
				layer.full(
					layer.open({
						...
						content: ['/hospExpLib/toView?HEL_ID=' + expertlibsOID+'&O_ID='+O_ID, 'yes'],//URL地址
						closeBtn: 1,//显示关闭按钮
						btn: ['关闭']
					})
				);
		...
		//浏览陪诊人员
		function accompanyDesc(accompanyinfoOID,O_ID){
			if(accompanyinfoOID != null && accompanyinfoOID != ""){
				layer.full(layer.open({
					...
					maxmin: true,//最大、小化是否显示
					scrollbar: true,//整体页面滚动条是否显示
					content: ['/accompanyInfo/toView?AI_ID=' + accompanyinfoOID+'&O_ID='+O_ID, 'yes'],//URL地址
					closeBtn: 1,//显示关闭按钮
					btn: ['关闭']
				}));
			}else{
