...
$(document).ready(function () {
    table= $('#queryTable').bootstrapTable({
	    url: '/jyb/secondMotion/pageSearch',
	    toolbar:'#toolbar' ,
	    columns: [
	    {
	        field: 'USER_ID',
		...
		function toAdd(){
			window.location.href="<%=basePath%>/jyb/secondMotion/toAdd";
		}
		...
		function toEdit(){
			...
			window.location.href="<%=basePath%>/jyb/secondMotion/toEdit?USER_ID="+ids[0];
		}
		...
		function toDel(){
			...
			layer.confirm('确认删除这些信息吗？',{
				...
				$.ajax({
					type: "POST",
					url: '/jyb/secondMotion/delete?tm='+ new Date().getTime(),
					data : {
						IDS : idsStr
					},
					dataType : 'json',
