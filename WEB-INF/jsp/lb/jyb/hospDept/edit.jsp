...
<body class="gray-bg">
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						<form id="ArticleRotationForm" name="ArticleRotationForm"
							class="form-horizontal" method="post">
							<div class="form-group">
								<!-- <label class="col-sm-2 control-label">TV青年班背景图：</label> -->
								<div class="col-sm-12">
									<label class="control-label">科室图标:</label>
									 <input id="inp" type="file" name="file" multiple class="file-loading">
									 <input type="hidden" name="FILE_UPLOAD_ID" id="FILE_UPLOAD_ID" value="${pd.FILE_UPLOAD_ID }">
									 <input type="hidden" name="OMS_ID" id="OMS_ID" value="${pd.OMS_ID} ">
									 <input type="hidden" name="I_IMAGE_ADDR" id="I_IMAGE_ADDR" value="${pd.I_IMAGE_ADDR }">
								</div>
							</div>
						</form>
					</div>
	...
	<script type="text/javascript">
		...
		if(keyID != null && keyID != ''){
			...
			var IDSAndPid=[];
			var ids;
			var MASTER_ID = keyID;//master_id(参数是举例，请根据实际传参)
		    ...
		    .on('fileuploaded', function(event, data, previewId, index) {//文件上传成功后
				...
				picturePATH = path;
				var picturess = JSON.parse(data.jqXHR.responseText)[0].path;
				...
				<%-- $.ajax({
					type: "POST",
					url: '<%=basePath%>filesUploads/updateMasterId',
					data: {
						MASTER_ID: MASTER_ID
					},
					dataType: 'json',
