...
<body class="gray-bg">
	...
	<script type="text/javascript">
		...
		//表格请求及数据
		var tableColumns = {
			url: 'articleHealth/pageSearch?CHECK_STATE=${pd.CheckState}&tm=' + new Date().getTime(),
			toolbar: '#toolbar',
			columns: [{
		        field: 'ARTICLE_ID',
		        visible: false,
		...
		function toCheck(){
			...
			layer.confirm('信息审核', {
				...
				btn1: function(index, layero){
					$.ajax({
						type: "POST",
						url: '<%=basePath%>articleHealth/toCheck?CHECK_STATE=${pd.CheckState + 1}&tm=' +  new Date().getTime(),
						data: {
							IDS: idsStr
						},
						dataType: 'json',
					...
					layer.open({
						...
						content: ['/articleHealth/toBackReason', 'no'],//URL地址
						closeBtn: 0,//不显示关闭按钮
						btn: ['确定', '取消'],
						btn1: function(index, layero){
							...
							if(backReason != null){
								$.ajax({
									type: "POST",
									url: '<%=basePath%>articleHealth/toCheck?CHECK_STATE=6&tm=' +  new Date().getTime(),
									data: {
										IDS: idsStr,
										Res: backReason
									},
