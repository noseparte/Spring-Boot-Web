...
<body class="gray-bg">
	...
	<script type="text/javascript">
		...
		//表格请求及数据
		var tableColumns = {
			url: 'articleRotation/pageSearch?tm=' + new Date().getTime(),
			toolbar: '#toolbar',
			columns: [{
		        field: 'ARTICLE_ID',
		        visible: false,
		...
		$(document).ready(function (){
			...
			if(msg != null && msg != ""){
				if(msg == 'success'){
					layer.msg("成功维护APP轮播信息！");
				}else{
					layer.msg("维护APP轮播信息发生异常，请重试或与管理员联系！");
				}
			}
			//初始化下拉菜单
		...
		//跳转到新增页面
		function toAdd(){
			window.location.href = "<%=basePath%>articleRotation/toAdd";
		}
		//跳转到编辑页面
		function toEdit(){
			...
			window.location.href = "<%=basePath%>articleRotation/toEdit?ARTICLE_ID=" + ids[0];
		}
		//批量删除数据
		function toDel(){
			var ids = getBstCheckedId('ARTICLE_ID');
			...
			layer.confirm('确定删除已选信息吗？', {
				...
				yes: function(index, layero){
					$.ajax({
						type: "POST",
						url: '<%=basePath%>articleRotation/toDelete?tm=' +  new Date().getTime(),
						data: {
							IDS: idsStr
						},
						dataType: 'json',
			...
			if(!flag){
				...
				layer.confirm('确定送审已选信息吗？', {
					...
					yes: function(index, layero){
						$.ajax({
							type: "POST",
							url: '<%=basePath%>articleRotation/toCheck?CHECK_STATE=1&tm=' +  new Date().getTime(),
							data: {
								IDS: idsStr
							},
							dataType: 'json',
		...
		function toViewBackReason(ARTICLE_ID){
			if(ARTICLE_ID != null && ARTICLE_ID != ""){
				layer.open({
					...
					content: ['/articleRotation/toBackReason?ARTICLE_ID=' + ARTICLE_ID + "&CL_TYPE=0&CL_LEVEL=6", 'no'],//URL地址
					closeBtn: 0,//显示关闭按钮
					btn: ['关闭']
				});
			}else{
			...
			if(ARTICLE_ID != null && ARTICLE_ID != "" && PUBLISH_STATE != null && PUBLISH_STATE != ""){
				...
				layer.confirm('确定' + alertMsg + '已选信息吗？', {
					...
					yes: function(index, layero){
						$.ajax({
							type: "POST",
							url: '<%=basePath%>articleRotation/toPublish?tm=' +  new Date().getTime(),
							data: {
